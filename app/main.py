...
async def predict_query(text: str = Query(..., min_length=1, max_length=1000),
    ...
    """Prediction with query parameters"""
...
async def get_predictions(limit: int = Query(default=10, le=100)):
    ...
    async with db_pool.acquire() as conn:
        rows = await conn.fetch(
            "SELECT request_id, input_text, sentiment, confidence, processing_time_ms, created_at FROM predictions ORDER BY created_at DESC LIMIT $1",
            limit
        )
...
async def get_analytics():
    ...
    return {
        ...
        "sentiment_distribution": [
            {'sentiment': r['sentiment'], 'count': r['c'], 'avg_confidence': float(r['a'])}
